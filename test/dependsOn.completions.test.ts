...
suite("dependsOn completions", () => {
    // function createDependsOnCompletionsTest(//asdf
    ...
    // function createResourceIdCompletionsTest2(//asdf
    ...
    suite("asdf", () => {
        ...
        function createDependsOnCompletionsTest(
            ...
            test(testName, async () => {
                ...
                assertEx.deepEqual(actual, options.expected, { ignorePropertiesNotInExpected: true });
        ...
        createDependsOnCompletionsTest(
            "detailMatchesFullInsertText",
            {
                ...
                expected: [
                    ...
                    {
                        "label": "[resourceId('type1', 'name1a')]",
                        "insertText": "[resourceId('type1', 'name1a')]",
                        replaceSpanText: "",
                        "kind": Completion.CompletionKind.dependsOnResName,
                        "detail": "detail: [resourceId('type1', 'name1a')]",
                        "documention": "asdf"
                    }
                ]
            });
        ...
        suite("expressions", () => {
            createDependsOnCompletionsTest(
                "type is expression",
                {
                    ...
                    expected: [
                        {
                            "label": "[resourceId(variables('type1'), 'name1a')]",
                            "insertText": "[resourceId('type1', 'name1a')]",
                            "documention": "asdf"
                        }
                    ]
            ...
            createDependsOnCompletionsTest(
                "name is expression",
                {
                    ...
                    expected: [
                        {
                            "label": "[resourceId('type1', concat(parameters('name1a'), 'foo'))]",
                            "insertText": "[resourceId('type1', concat(parameters('name1a'), 'foo'))]",
                            "documention": "asdf"
                        }
                    ]
                }
        ...
        suite("completion triggering", () => {
            ...
            createDependsOnCompletionsTest(
                "replacement includes everything to left of cursor",
                {
                    template: {
                        resources: [
                            {
        ...
        suite("asdf multiple resources", () => {
            test("asdf multiple resources");
            test("asdf doesn't include ref to self");
            test("asdf *does* include existing dependsOn references ???");
            test("case insensitive dependsOn");
        });
        ...
        createDependsOnCompletionsTest(
            "asdf",
            {
                ...
                expected: [
                    ...
                    {
                        "label": "[resourceId('type1', 'name1b')]",
                        "insertText": "[resourceId('type1', 'name1b')]",
                        replaceSpanText: ""
                    },
                    {
                        "label": "name2",
